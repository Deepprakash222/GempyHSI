...
def main():
    ...
    directory_path = directory_path + "/" + dataset  + "_slope_gempy_" + str(slope_gempy)  + "_likelihood_std_" + str(likelihood_std)
    ...
    if dataset=="Syn_label":
        ...
        plt.close()
        obs_data = torch.tensor(labels, dtype=torch.float64)
    elif dataset=="Syn_label_shift_20_error":
        ...
        for label_ in np.unique(label):
            idx =label ==label_
            plt.scatter(z[idx],label[idx], c=label_to_color[label_],label=f' {label_}',s=50, marker='o',alpha=1.0, edgecolors='w')
        # Create a legend
        plt.legend()
        # Add axis labels
        plt.xlabel('Depth')
        ...
        plt.close()
        # Convert to tensor
        obs_data = torch.tensor(new_label, dtype=torch.float64)
