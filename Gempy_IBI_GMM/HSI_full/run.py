...
def main():
    ...
    directory_path = directory_path + "/" + dataset +  "_slope_gempy_" + str(slope_gempy) + "_scale_" + str(scale) + "_chain_" + str(posterior_num_chain) + "_warmup_step_" + str(posterior_warmup_steps) + "_posterior_sample_" + str(posterior_number_samples)
    ...
    test_list=[]
    if dataset=="Salinas":
        test_list.append({"update":"interface_data","id":torch.tensor([1]), "direction":"Z", "prior_distribution":"normal","normal":{"mean":torch.tensor(sp_coords_copy_test[1,2],device =device, dtype =dtype), "std":torch.tensor(2.0,device =device, dtype =dtype)}})
        test_list.append({"update":"interface_data","id":torch.tensor([4]), "direction":"Z", "prior_distribution":"normal","normal":{"mean":torch.tensor(sp_coords_copy_test[4,2],device =device, dtype =dtype), "std":torch.tensor(2.0,device =device, dtype =dtype)}})
        test_list.append({"update":"interface_data","id":torch.tensor([7]), "direction":"Z", "prior_distribution":"normal","normal":{"mean":torch.tensor(sp_coords_copy_test[7,2],device =device, dtype =dtype), "std":torch.tensor(2.0,device =device, dtype =dtype)}})
        test_list.append({"update":"interface_data","id":torch.tensor([12]), "direction":"Z", "prior_distribution":"normal","normal":{"mean":torch.tensor(sp_coords_copy_test[12,2],device =device, dtype =dtype), "std":torch.tensor(2.0,device =device, dtype =dtype)}})
    elif dataset =="KSL":
        test_list.append({"update":"interface_data","id":torch.tensor([1]), "direction":"Z", "prior_distribution":"normal","normal":{"mean":torch.tensor(sp_coords_copy_test[1,2],device =device, dtype =dtype), "std":torch.tensor(0.02,device =device, dtype =dtype)}})
        test_list.append({"update":"interface_data","id":torch.tensor([4]), "direction":"Z", "prior_distribution":"normal","normal":{"mean":torch.tensor(sp_coords_copy_test[4,2],device =device, dtype =dtype), "std":torch.tensor(0.02,device =device, dtype =dtype)}})
        test_list.append({"update":"interface_data","id":torch.tensor([7]), "direction":"Z", "prior_distribution":"normal","normal":{"mean":torch.tensor(sp_coords_copy_test[7,2],device =device, dtype =dtype), "std":torch.tensor(0.02,device =device, dtype =dtype)}})
    elif dataset=="KSL_layer3":
        test_list.append({"update":"interface_data","id":torch.tensor([1]), "direction":"Z", "prior_distribution":"normal","normal":{"mean":torch.tensor(sp_coords_copy_test[1,2],device =device, dtype =dtype), "std":torch.tensor(0.02,device =device, dtype =dtype)}})
        test_list.append({"update":"interface_data","id":torch.tensor([4]), "direction":"Z", "prior_distribution":"normal","normal":{"mean":torch.tensor(sp_coords_copy_test[4,2],device =device, dtype =dtype), "std":torch.tensor(0.02,device =device, dtype =dtype)}})
